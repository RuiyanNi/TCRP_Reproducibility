...
class MetaLearner(object):
	...
	def zero_shot_test(self, unseen_test_loader):
		...
		mtest_loss, mtest_pear_corr, mtest_spearman_corr, _, _ = evaluate_new( unseen_tissue_model, unseen_test_loader, 0 )

		return mtest_loss, mtest_pear_corr, mtest_spearman_corr
	...
	def unseen_tissue_learn(self, unseen_train_loader, unseen_test_loader):
		...
		# when 1, use training set for fintuning; when 0, testing only
		mtrain_loss, mtrain_pear_corr, mtrain_spearman_corr, train_prediction, train_true_label = evaluate_new( unseen_tissue_model, unseen_train_loader,1 )
		...
		return mtrain_loss, mtrain_pear_corr, mtrain_spearman_corr, mtest_loss, mtest_pear_corr, mtest_spearman_corr, test_prediction, test_true_label, unseen_tissue_model
	...
	def train(self):
		...
		# uncomment this to do k shot learning
		...
		# for pretrained only (k=0)
		unseen_zero_test_loader = get_zero_shot_unseen_data_loader( self.fs_dataset.feature, self.fs_dataset.label)
		...
		test_pred = [[0 for i in range(len(unseen_test_loader))] for j in range(self.num_updates)]

		zero_test_pear_corr, zero_test_spearman_corr = np.zeros((self.num_updates,)), np.zeros((self.num_updates,))
		best_zero_pear_corr, best_zero_spearman_corr = 0, 0
		...
		for epoch in range( self.num_updates ):
			...
			# zero-shot learning
			zero_test_loss, zero_test_pear_corr[epoch], zero_test_spearman_corr[epoch] = self.zero_shot_test(unseen_zero_test_loader)
			...
			# print('HAHAHAHAHA', meta_train_loss_mean, meta_train_corr_mean, meta_val_loss_mean, meta_val_corr_mean)

			## Evaluate K shot test tasks
			train_loss[epoch], train_corr[epoch], train_spearman_corr[epoch], test_loss[epoch], test_corr[epoch], test_spearman_corr[epoch], test_prediction, test_true_label, unseen_tissue_model = self.unseen_tissue_learn( unseen_train_loader, unseen_test_loader )
			
			# save test_prediction
			test_pred[epoch] = test_prediction
			...
			if train_corr[epoch] > best_fewshot_train_corr:
				...
				# save the best unseen model
				best_unseen_tissue_model = unseen_tissue_model
			...
			best_zero_pear_corr = zero_test_pear_corr[0]
			best_zero_spearman_corr = zero_test_spearman_corr[0]
		...
		best_train_corr_test_corr = test_corr[ best_train_corr_epoch ] #pearson
		best_train_corr_test_scorr = test_spearman_corr[ best_train_corr_epoch ]
		best_train_scorr_test_scorr = test_spearman_corr[ best_train_spearman_corr_epoch ] #spearman

		# save best test prediction - pearson and spearman
		best_test_pred_pear = test_pred[best_train_corr_epoch].detach().cpu().numpy()
		best_test_pred_spear = test_pred[best_train_spearman_corr_epoch].detach().cpu().numpy()
		test_true_label = test_true_label.detach().cpu().numpy()

		# self.fs_dataset.feature, self.fs_dataset.label
		
		# generate predicted drug response 
		best_unseen_tissue_model.eval()
		out_pred = np.zeros((self.fs_dataset.label.shape[0],))

		for i, in_ in enumerate(self.fs_dataset.feature):
			in_ = torch.FloatTensor(in_).cuda()
			input_var = Variable(in_)
			# target_var = Variable(target)

			# Second output is hidden
			out, _ = best_unseen_tissue_model(input_var)
			# print('prediction: ', str(out))
			out = out.detach().cpu().numpy()
			out_pred[i] = out

		
		print '--trial summerize--', 'best_train_loss_test_corr:', float('%.3f'%best_train_loss_test_corr), 'best_train_corr_test_corr', float('%.3f'%best_train_corr_test_corr), 'best_train_corr_test_scorr', float('%.3f'%best_train_corr_test_scorr), 'best_train_scorr_test_scorr', float('%.3f'%best_train_scorr_test_scorr)
		
		return best_train_loss_test_corr, best_train_corr_test_corr, best_train_corr_test_scorr, best_train_scorr_test_scorr, best_train_corr_model, best_test_pred_pear, best_test_pred_spear, test_true_label, out_pred, best_zero_pear_corr, best_zero_spearman_corr
